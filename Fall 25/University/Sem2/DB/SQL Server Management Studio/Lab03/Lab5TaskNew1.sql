...
-- Query 1. Apply all of the character and string functions and report the output --
	...
	SELECT *, UPPER(LEFT(Employee_Name, 1))+
		LOWER(SUBSTRING(Employee_Name, 2 , LEN(Employee_Name))) AS 'First Letter Cap'
			FROM Employee;
	...
	*/
	...
	------------------------------------------------------------------------------------------------------------------
		-- Query 3: Apply a query to print the names and joining date of all of the employees in such a way that if I have entred
		--			some employee's name like "muHammad waQar" then the query should return it as Muhammad Waqar. --
	------------------------------------------------------------------------------------------------------------------
			
	SELECT Employee_Name, 
		UPPER(LEFT(Employee_Name, 1))+
		LOWER(SUBSTRING(Employee_Name, 2, CHARINDEX(' ', Employee_Name)-1))+
		UPPER(SUBSTRING(Employee_Name, CHARINDEX(' ', Employee_Name), 2))+
		LOWER(SUBSTRING(Employee_Name, CHARINDEX(' ', Employee_Name, 3)+2, LEN(Employee_Name)))
		AS 'FOrmated Employee_Name', Joining_Date
			FROM Employee;
